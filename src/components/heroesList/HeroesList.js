...
const HeroesList = () => {
  ...
  const onDelete = useCallback(
    (id) => {
      request(`https://run.mocky.io/v3/501f642a-e792-42c0-817a-00aa7c440025/${id}`, 'DELETE')
  ...
  const renderHeroesList = (arr) => {
    if (arr.length === 0) {
      <CSSTransition timeout={0} classNames="hero-list__items">
    ...
    return arr.map(({ id, ...props }) => {
      return (
        <CSSTransition classNames="hero-list__items" timeout={500} key={id}>
